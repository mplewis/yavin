...
describe('with a document and keyword list', () => {
  ...
  describe('evaluateListWords', () => {
    it('analyzes a document for word hits', () => {
      ...
      keywordLists.forEach((list) => {
        results[list.name] = evaluateListWords(bodyWc, list);
      });
  ...
  describe('evaluateListPhrases', () => {
    it('analyzes a document for phrase hits', () => {
      const results: { [name: string]: number } = {};
      keywordLists.forEach((list) => {
        results[list.name] = evaluateListPhrases(bodyWc, list);
      });
      expect(results).toMatchInlineSnapshot(`
        Object {
          "conspiracy": 1,
  ...
  describe('evaluateList', () => {
    it('analyzes a document for hits', () => {
      const results: {
        [name: string]: { hits: number; frequency: number };
      } = {};
      keywordLists.forEach((list) => {
        results[list.name] = evaluateList(bodyWc, list);
      });
      expect(results).toMatchInlineSnapshot(`
        Object {
          "conspiracy": Object {
            "frequency": 0.06521739130434782,
            "hits": 3,
          },
          "fraud": Object {
            "frequency": 0,
  ...
  describe('tag', () => {
    it('tags a document based on hit frequency', () => {
      expect(tag(body, keywordLists)).toEqual(['theft']);
    });
  });
});
...
describe('with the real keyword list and real emails', () => {
  ...
  it('tags emails as expected', () => {
    ...
    emails.forEach((email) => {
      ...
      const tags = tag(body, keywordLists);
